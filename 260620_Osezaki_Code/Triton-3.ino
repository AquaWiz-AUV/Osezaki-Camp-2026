/**
 * @file Triton-3-v3-sensor-lockout-disabled.ino
 * @brief Triton-3 v3 firmware using the protocol_v36 TWELITE wireless protocol.
 *
 ...
 *
 * v3 keeps the Triton-3-v2 hardware integration for GPS, RTC, TSYS01, MS5837,
 * SD logging, optional LCD, LEDs, and the two valves. Operator control is moved
 * from the OE13KIR infrared mode buttons to the v3.6 wireless ControlPlan
 * protocol:
 *
...
const unsigned long DEFAULT_DATA_LOG_MS = 1000UL;
...
void loop() {
  ...
  if (controlState == STATE_RUNNING) {
    logIntervalMs = static_cast<uint32_t>(runningPlan.logInterval100ms) * 100UL;
...
bool initializeTempSensor() {
  ...
  if (!tempSensor.init() || wireTimeoutFlagged()) {
...
void updateTempSensor() {
  ...
  sensorData.tempValid = !isnan(sensorData.waterTemperature) &&
...
int16_t currentWaterTempCentiC() {
  if (!sensorData.tempValid) return static_cast<int16_t>(0x8000);
  float centiFloat = sensorData.waterTemperature * 100.0f;
...
uint8_t statusFlags() {
  ...
  if (tempReady && sensorData.tempValid) flags |= STATUS_TEMP_OK;
...
void sendStatusIfDue() {
  ...
  if (controlState == STATE_RUNNING) {
    interval = runningPlan.statusInterval100ms;
  } else if (loadedPlanValid) {
    interval = loadedPlan.statusInterval100ms;
  }
  if (interval == 0) interval = 10;
  if (currentMillis - lastStatusMs >= static_cast<uint32_t>(interval) * 100UL) {
    sendStatus();
...
void updateDisplay() {
  ...
  lcd.print(F("T:"));
  if (sensorData.tempValid) {
    lcd.print(sensorData.waterTemperature, 1);
  } else {
    lcd.print(F("NA"));
  }
  lcd.print(F(" D:"));
