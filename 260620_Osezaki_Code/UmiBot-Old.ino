 ...
 *
 *   PREP 5 s -> EXH 90 s -> DESC 5 s -> WAIT 55 s -> INJ 20 s -> ASC 90 s
 *
...
const uint8_t INTERNAL_PRESSURE = A3;
...
const uint8_t VALVE_PRESSURE = 5;  // old V3 pressure equalization
const uint8_t GPS_RX = 3;          // matches old SoftwareSerial(3, 2)
const uint8_t GPS_TX = 2;
}

namespace Timing {
const uint16_t PREPARE_S = 5;
const uint16_t EXHAUST_OPEN_S = 90;
const uint16_t DESCENT_COAST_S = 5;
const uint16_t BOTTOM_WAIT_S = 55;
const uint16_t INJECTION_OPEN_S = 20;
const uint16_t ASCENT_WAIT_S = 90;

const uint16_t LOG_INTERVAL_100MS = 10;
const uint16_t STATUS_INTERVAL_100MS = 10;
const unsigned long DATA_LOG_MS = LOG_INTERVAL_100MS * 100UL;
...
constexpr uint16_t PLAN_REPEAT_COUNT = 0xFFFF;
constexpr uint16_t PLAN_MAX_RUNTIME_MIN = 360;
constexpr uint16_t PLAN_DEPTH_TRIGGER_CM = 0;
constexpr uint16_t PLAN_MAX_DEPTH_CM = 3500;
...
constexpr uint16_t INTERNAL_PRESS_THRESH_MBAR = 5;
...
struct SensorData {
  ...
  float internalPressureMbar = NAN;
};
...
void updateInternalPressureValve();
void startPlan();
...
void setPressureValve(bool pressureOn);
void logData();
...
void loop() {
  ...
  if (currentMillis - lastDataLogMs >= Timing::DATA_LOG_MS) {
    lastDataLogMs = currentMillis;
    logData();
  }
...
void updateInternalPressureValve() {
  if (!sensorData.depthValid) {
    sensorData.internalPressureMbar = NAN;
    setPressureValve(false);
    return;
  }

  float raw = analogRead(Pins::INTERNAL_PRESSURE);
  float voltage = raw * 0.00488f - 0.25f;
  float gaugeBar = voltage * 6.667f;
  float internalAbsMbar = gaugeBar * 68.94f + 1013.25f;
  sensorData.internalPressureMbar = internalAbsMbar;

  float diffMbar = internalAbsMbar - sensorData.pressureMbar;
  setPressureValve(diffMbar + INTERNAL_PRESS_THRESH_MBAR < 0.0f);
}
...
void updateRunningState() {
  ...
  if (PLAN_MAX_RUNTIME_MIN > 0 &&
      currentMillis - runningStartedMs >= static_cast<uint32_t>(PLAN_MAX_RUNTIME_MIN) * 60000UL) {
...
void applyValves(bool injectionOn, bool exhaustOn, const char* cause) {
  if (injectionOn && exhaustOn) {
    injectionOn = false;
    exhaustOn = false;
    errorFlags |= ERR_VALVE_CONFLICT;
    logEvent("VALVE_CONFLICT", PLAN_ID, PLAN_DEPTH_TRIGGER_CM, cause);
  }
...
void setPressureValve(bool pressureOn) {
  if (valveStatus.pressureValve == pressureOn) return;
  digitalWrite(Pins::VALVE_PRESSURE, pressureOn ? HIGH : LOW);
  valveStatus.pressureValve = pressureOn;
}
...
void updateLEDs() {
  bool sequenceRunning = controlState == STATE_RUNNING && (errorFlags & ERR_VALVE_CONFLICT) == 0;
  bool hasWarning = (errorFlags & (ERR_SD_WRITE | ERR_TEMP_SENSOR | ERR_DEPTH_SENSOR | ERR_SENSOR_STALE | ERR_VALVE_CONFLICT)) != 0;
...
const char* stateToString(uint8_t value) {
  switch (value) {
    ...
    case STATE_PLAN_LOADED: return "LOAD";
